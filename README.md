...
**heavybad.py v1.0.4** — Multi-pass bad-sector detector for Linux raw block devices.
...
- **Resume** — saves progress to `heavybad.resume` every 1000 chunks and on Ctrl+C, restoring counts and position on next run. Resume state is matched per scan step (device, range, chunk size, and mode), so a queue with multiple scan types resumes the correct one
...
If a queue run is interrupted, rerunning `queue.json` with `--resume`/`"resume": true` will skip any scans that already completed and resume the interrupted one from where it left off — resume state is matched against each scan's device, range, chunk size, and mode, so it can't be misapplied to a different scan step.
